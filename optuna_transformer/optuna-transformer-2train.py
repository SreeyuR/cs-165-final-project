...
config_file_path = "/groups/tensorlab/sratala/fno-disruption-pred/config/transformer/config_transformer_2train_fixed.yaml" 
study_name = 'A-2TRAIN_TRANSFORMER_AUC_MAX' #'TEST-TRANSFORMER_3_REACTORS_AUC_MAX'
wandb_proj_name = 'CORRECT-Transformer_Disruption_Tuning' #'Test-Transformer_Disruption_Tuning'
wandb_group_name = '2train-transformer-tuning' # 'test-transformer-tuning'
...
def sample_hyperparameters(trial):
    ...
    delta_t = trial.suggest_int('delta_t_factor', 3, 19) * 50 #trial.suggest_int('delta_t', 10, 70)
    ...
    return {
        'training': {
            ...
            'learning_rate_scheduler_target': trial.suggest_float('learning_rate_scheduler_target', 178e-6, 0.016, log=True), # higher initial learning rate does significantly better 
            'learning_rate_adam': trial.suggest_float('learning_rate_adam', 1e-3, 0.2, log=True),
            'learning_rate_init': trial.suggest_float('learning_rate_init', 1.2e-6, 1e-3, log=True),
            ...
            'step_size': trial.suggest_int('step_size', 13, 40),
            ...
            # is reached. 
            'gamma': trial.suggest_float('gamma', 0.25, 0.5), # 0.1, 0.8
            'warmup_steps': trial.suggest_int('warmup_steps', 12, 25),
            # Adam
            'adam_weight_decay': trial.suggest_float('weight_decay', 16e-6, 610e-6, log=True), #  40e-6, 1.5e-3
        ...
        'transformer': {
            'n_inner': trial.suggest_int('n_inner', 15, 35),             # from minimal to large inner layer size
            # 'activation_function': trial.suggest_categorical('activation_function', ['relu', 'gelu', 'tanh']),
            'attn_pdrop': trial.suggest_float('attn_pdrop', 0.1, 0.29),
            'resid_pdrop': trial.suggest_float('resid_pdrop', 0.0076, 0.2603),
            'embd_pdrop': trial.suggest_float('embd_pdrop', 0.09, 0.2),
            'layer_norm_epsilon': trial.suggest_float('layer_norm_epsilon', 8e-6, 1e-4, log=True),
        ...
        'data': {
            ...
            'end_cutoff_timesteps': trial.suggest_int('end_cutoff_timesteps', 1, 7), 
        ...
        'balance_classes': {
            ...
            'undersample_ratio': trial.suggest_float('undersample_ratio', 1.5, 4.6), # lower the number, higher % of disruptions in dataset
            # in addition to ratio d:nd weight in loss function, add'l emphasis multiplied to pos class
            'pos_weight_emphasis_factor': trial.suggest_float('pos_weight_emphasis_factor', 1.25, 2.78), # 1.6
