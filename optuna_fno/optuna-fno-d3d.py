...
config_file_path = "/groups/tensorlab/sratala/fno-disruption-pred/config/fno/config_d3d_fno.yaml"
study_name = 'A-d3d-fno-auc'
addl_metric = True
wandb_proj_name = "CORRECT-d3d-FNO-Disruption-Tuning"
wandb_group_name = 'd3d-fno-tuning'
metrics = ['auc_score']
...
def sample_hyperparameters(trial):
    ...
    delta_t = trial.suggest_int('delta_t_factor', 1, 16) * 50 #trial.suggest_int('delta_t', 10, 70)
    ...
    max_n_modes = min(max_n_modes, 14)
    return {
        'training': {
            #'batch_size': trial.suggest_categorical('batch_size', [8, 16, 32, 64]), #trial.suggest_int('batch_size', 17, 32),
            # 'learning_rate': trial.suggest_float('learning_rate', 1e-6, 1e-1, log=True),
            'learning_rate_scheduler_target': trial.suggest_float('learning_rate_scheduler_target', 0.0015, 0.082, log=True), # higher initial learning rate does significantly better 
            'learning_rate_adam': trial.suggest_float('learning_rate_adam', 0.0013, 0.095, log=True),
            'learning_rate_init': trial.suggest_float('learning_rate_init', 15e-6, 377e-6, log=True),
            ...
            # is reached. 
            'gamma': trial.suggest_float('gamma', 0.212, 0.8), # 0.1, 0.8
            'warmup_steps': trial.suggest_int('warmup_steps', 13, 39),
            # Adam
            'adam_weight_decay': trial.suggest_float('weight_decay', 1.8e-6, 9.9e-3, log=True), #  40e-6, 1.5e-3
            ...
            'hidden_channels': trial.suggest_int('hidden_channels', 15, 38), # (30, 50)
            'mlp_dropout': trial.suggest_float('mlp_dropout', 0.036, 0.47), # (0.15, 0.4)
        },
        'data': {
        ...
        'balance_classes': {
            ...
            'undersample_ratio': trial.suggest_float('undersample_ratio', 1.342, 4.24), # lower the number, higher % of disruptions in dataset
            # in addition to ratio d:nd weight in loss function, add'l emphasis multiplied to pos class
            'pos_weight_emphasis_factor': trial.suggest_float('pos_weight_emphasis_factor', 1.445, 3.94), # 1.6
