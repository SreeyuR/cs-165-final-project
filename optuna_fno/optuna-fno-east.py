...
config_file_path = "/groups/tensorlab/sratala/fno-disruption-pred/config/fno/config_east_fno.yaml"
study_name = 'A-east-fno-auc'
addl_metric = True
wandb_proj_name = "CORRECT-east-FNO-Disruption-Tuning"
wandb_group_name = 'east-fno-tuning'
metrics = ['auc_score']
...
def sample_hyperparameters(trial):
    ...
    delta_t = trial.suggest_int('delta_t_factor', 2, 15) * 50 #trial.suggest_int('delta_t', 10, 70)
    ...
    max_n_modes = min(max_n_modes, 9)
    return {
        'training': {
            #'batch_size': trial.suggest_categorical('batch_size', [8, 16, 32, 64]), #trial.suggest_int('batch_size', 17, 32),
            # 'learning_rate': trial.suggest_float('learning_rate', 1e-6, 1e-1, log=True),
            'learning_rate_scheduler_target': trial.suggest_float('learning_rate_scheduler_target', 0.018, 0.15, log=True), # higher initial learning rate does significantly better 
            'learning_rate_adam': trial.suggest_float('learning_rate_adam', 0.0016, 0.03, log=True),
            'learning_rate_init': trial.suggest_float('learning_rate_init', 11.88e-6, 110.4e-6, log=True),
            ...
            'step_size': trial.suggest_int('step_size', 21, 39),
            ...
            # is reached. 
            'gamma': trial.suggest_float('gamma', 0.7, 0.84), # 0.1, 0.8
            'warmup_steps': trial.suggest_int('warmup_steps', 11, 21),
            # Adam
            'adam_weight_decay': trial.suggest_float('weight_decay', 2.57e-6, 9.97e-2, log=True), #  40e-6, 1.5e-3
            ...
            'hidden_channels': trial.suggest_int('hidden_channels', 10, 15), # (30, 50)
            'mlp_dropout': trial.suggest_float('mlp_dropout', 0.11, 0.21), # (0.15, 0.4)
        },
        'data': {
            # cutoff of dataset, before truncation or anything
            'end_cutoff_timesteps': trial.suggest_int('end_cutoff_timesteps', 4, 6), 
        ...
        'balance_classes': {
            ...
            'undersample_ratio': trial.suggest_float('undersample_ratio', 1.8, 2.73), # lower the number, higher % of disruptions in dataset
            # in addition to ratio d:nd weight in loss function, add'l emphasis multiplied to pos class
            'pos_weight_emphasis_factor': trial.suggest_float('pos_weight_emphasis_factor', 3.1, 3.59), # 1.6
