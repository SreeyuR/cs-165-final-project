...
config_file_path = "/groups/tensorlab/sratala/fno-disruption-pred/config/fno/config_cmod_fno_recall_pos_weight.yaml"
study_name = 'RECALL-cmod-fno-auc'
addl_metric = True
wandb_proj_name = "RECALL-POS-WEIGHT-CORRECT-CMOD-FNO-Disruption"
wandb_group_name = 'cmod-fno-tuning'
metrics = ['recall']
...
def sample_hyperparameters(trial):
    ...
    max_n_modes = min(max_n_modes, 5)
    return {
        # 'training': {
            #'batch_size': trial.suggest_categorical('batch_size', [8, 16, 32, 64]), #trial.suggest_int('batch_size', 17, 32),
        #     # 'learning_rate': trial.suggest_float('learning_rate', 1e-6, 1e-1, log=True),
        #     'learning_rate_scheduler_target': trial.suggest_float('learning_rate_scheduler_target', 290e-6, 0.01, log=True), # higher initial learning rate does significantly better 
        #     'learning_rate_adam': trial.suggest_float('learning_rate_adam', 0.001, 0.0034, log=True),
        #     'learning_rate_init': trial.suggest_float('learning_rate_init', 90e-6, 100e-6, log=True),
        ...
        #     'step_size': trial.suggest_int('step_size', 20, 30),
        ...
        #     # is reached. 
        #     'gamma': trial.suggest_float('gamma', 0.2, 0.77), # 0.1, 0.8
        #     'warmup_steps': trial.suggest_int('warmup_steps', 15, 30),
        #     # Adam
        #     'adam_weight_decay': trial.suggest_float('weight_decay', 2.38e-6, 145e-6, log=True), #  40e-6, 1.5e-3
        ...
        #     'hidden_channels': trial.suggest_int('hidden_channels', 15, 47), # (30, 50)
        #     'mlp_dropout': trial.suggest_float('mlp_dropout', 0.007, 0.42), # (0.15, 0.4)
        # },
        # 'data': {
        #     # cutoff of dataset, before truncation or anything
        #     'end_cutoff_timesteps': trial.suggest_int('end_cutoff_timesteps', 0, 7), 
        ...
        'balance_classes': {
            ...
            #'undersample_ratio': trial.suggest_float('undersample_ratio', 2.6, 3.06), # lower the number, higher % of disruptions in dataset
            # in addition to ratio d:nd weight in loss function, add'l emphasis multiplied to pos class
            'pos_weight_emphasis_factor': trial.suggest_float('pos_weight_emphasis_factor', 0.0, 4.6), # 1.6
